...
"#;
    ...
    #[test]
    fn parses_and_validates_sample() -> Result<(), Box<dyn Error>> {
        let config: ProjectConfig = toml::from_str(SAMPLE)?;
        config.validate()?;
    ...
    #[test]
    fn rejects_unknown_provider() -> Result<(), Box<dyn Error>> {
        let broken = SAMPLE.replace("provider = \"nearblocks\"", "provider = \"missing\"");
        let config: ProjectConfig = toml::from_str(&broken)?;
        assert!(matches!(
            config.validate(),
