...
mod tests {
    ...
    #[test]
    fn timestamps_normalise() -> Result<(), Box<dyn Error>> {
        assert_eq!(
            normalise_timestamp("2021-01-02 03:04:05")?,
            "2021-01-02T03:04:05Z"
        );
        assert_eq!(
            normalise_timestamp("2021-01-02 03:04:05.1234")?,
            "2021-01-02T03:04:05.1234Z"
        );
        assert_eq!(
            normalise_timestamp("2021-01-02T03:04:05Z")?,
            "2021-01-02T03:04:05Z"
        );
        assert_eq!(
            normalise_timestamp("1/2/2021 03:04")?,
            "2021-01-02T03:04:00Z"
        );
        assert_eq!(normalise_timestamp("2021-01-02")?, "2021-01-02T00:00:00Z");
        assert!(normalise_timestamp("garbage").is_err());
        Ok(())
    }

    #[test]
    fn amounts_parse() -> Result<(), Box<dyn Error>> {
        assert_eq!(parse_amount("1,234.56")?.to_string(), "1234.56");
        assert_eq!(parse_amount("+0.5")?.to_string(), "0.5");
        assert_eq!(parse_amount("-2")?.to_string(), "-2");
