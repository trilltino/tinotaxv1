...
use tinotax_store::{
    hash_file, Cursor, EndpointCache, ProjectPaths, RawFileManifestEntry, RawManifest,
};
use tracing::info;
    ...
    async fn fetch_endpoint(
        ...
        loop {
            ...
            let (body, reused_cached_page) = if ctx.resume && existing_page.exists() {
                let text = std::fs::read_to_string(&existing_page)
                    .with_context(|| format!("reading cached page {existing_page}"))?;
                let body: serde_json::Value = serde_json::from_str(&text)
                    .with_context(|| format!("parsing cached page {existing_page}"))?;
                (body, true)
            } else {
            ...
            if item_count > 0 {
                let (rel_path, hash) = if reused_cached_page {
                    let (hash, _) = hash_file(&existing_page)?;
                    (paths.relative(&existing_page), hash)
                } else {
                    cache.write_page(page, &body)?
                };
