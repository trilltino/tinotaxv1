...
/// 429s come from per-minute rate windows, so sub-minute exponential backoff
/// just burns attempts; wait out the window instead. Escalating linearly
/// means a daily-quota 429 still fails within a few minutes rather than
/// spinning forever.
const RATE_LIMIT_BACKOFF_MS: u64 = 35_000;
    ...
    async fn get_json_with_attempts(
        ...
        loop {
            ...
            let backoff = if rate_limited {
                Duration::from_millis(RATE_LIMIT_BACKOFF_MS * u64::from(attempt))
            } else {
