...
pub enum ProviderSpec {
    ...
    NearBlocks {
        ...
        /// Rate-limit budget of the account's API plan (NearBlocks bills one
        /// credit per 25 items). Defaults to the free plan when `None`.
...
pub fn make_fetcher(spec: &ProviderSpec) -> anyhow::Result<Box<dyn WalletFetcher>> {
    ...
    Ok(match spec {
        ...
        ProviderSpec::NearBlocks {
            base_url,
            api_key,
            credits_per_minute,
        } => Box::new(NearBlocksFetcher::new(
            base_url.clone(),
            api_key.clone(),
            *credits_per_minute,
            http,
