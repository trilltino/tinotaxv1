...
use cli::{
    ...
    ProjectWorkflowCommand, ReviewCommand,
};
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Command::Preflight { config, project } => tinotax_app::preflight(&config, &project),
        ...
        Command::Project { command } => match command {
            ...
            ProjectCommand::Status { project } => tinotax_app::project_status(&project.project),
            ProjectCommand::Paths { project, tax_year } => {
                tinotax_app::project_paths(&project.project, tax_year.as_deref())
            }
            ProjectCommand::Clean {
                ...
                let targets = target.into_iter().map(Into::into).collect::<Vec<_>>();
                tinotax_app::project_clean(&project.project, &targets, tax_year.as_deref(), confirm)
            }
            ProjectCommand::Workflow { command } => match command {
                ProjectWorkflowCommand::Startup {
                    config,
                    project,
                    resume,
                } => tinotax_app::workflow_startup(&config, &project, resume).await,
                ProjectWorkflowCommand::RefreshReview { project } => {
                    tinotax_app::workflow_refresh_review(&project.project)
                }
                ProjectWorkflowCommand::FinalizeYear {
                    project,
                    tax_year,
                    allow_unpriced,
                } => {
        ...
        Command::Readiness { project } => tinotax_app::readiness(&project.project),
