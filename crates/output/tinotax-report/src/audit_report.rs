...
pub fn write_audit_manifest(paths: &ProjectPaths, project_name: &str) -> Result<()> {
    ...
    if out_dir.exists() {
        ...
        for entry in std::fs::read_dir(&out_dir).with_context(|| format!("listing {out_dir}"))? {
            let entry = entry.with_context(|| format!("reading entry in {out_dir}"))?;
            let file_type = entry
                .file_type()
                .with_context(|| format!("reading file type for {:?}", entry.path()))?;
