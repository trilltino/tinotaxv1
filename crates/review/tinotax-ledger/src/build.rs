...
mod tests {
    ...
    #[test]
    fn machine_suggestion_without_override() -> Result<(), Box<dyn Error>> {
        let row = to_ledger_event(&sample_event(), None)?;
    ...
    #[test]
    fn user_tax_type_wins_over_action_and_suggestion() -> Result<(), Box<dyn Error>> {
        ...
        let row = to_ledger_event(&sample_event(), Some(&o))?;
