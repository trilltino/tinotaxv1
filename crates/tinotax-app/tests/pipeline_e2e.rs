...
#[test]
fn full_pipeline_from_normalised_events_to_evidence_pack() {
    let tmp = tempfile::tempdir().unwrap();
    let root = Utf8PathBuf::from_path_buf(tmp.path().join("proj")).unwrap();
    let project = root.as_str();

    // A project as `normalise` would have left it.
    let paths = ProjectPaths::new(root.clone());
    paths.init().unwrap();
    std::fs::write(paths.config_file(), PROJECT_TOML).unwrap();
    let mut writer = JsonlWriter::create(&paths.events_jsonl()).unwrap();
    for e in [
        event(
            "buy",
    ...
    let rows = tinotax_app::export_review_all(project).unwrap();
    assert_eq!(rows, 3);
    ...
    .unwrap();
    let applied = tinotax_app::apply_review(project, edited.as_str()).unwrap();
    assert_eq!(applied, 1);
    assert!(paths.out().join("change_log.csv").exists());

    // 3. Build the reviewed ledger.
    tinotax_app::ledger_build(project).unwrap();
    ...
    tinotax_app::prices_missing(project).unwrap();
    let missing = std::fs::read_to_string(paths.out().join("missing_prices.csv")).unwrap();
    ...
    .unwrap();
    tinotax_app::prices_import(project, prices.as_str()).unwrap();

    // 5. Price the ledger.
    tinotax_app::ledger_price(project).unwrap();
    ...
    tinotax_app::calculate_uk(project, "2024-2025", false).unwrap();
    ...
    // 7. Evidence pack.
    tinotax_app::pack_hmrc(project, "2024-2025").unwrap();
    ...
    let disposals = std::fs::read_to_string(pack.join("disposals_calculation.csv")).unwrap();
    assert_eq!(
