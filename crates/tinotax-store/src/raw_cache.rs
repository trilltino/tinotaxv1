...
mod tests {
    ...
    #[test]
    fn roundtrips_pages_and_cursor() {
        ...
        let (rel, hash) = cache.write_page(1, &body).unwrap();
        assert_eq!(rel, "raw/near/foxboss.near/transactions/page_000001.json");
        assert_eq!(hash.len(), 64);

        assert!(cache.read_cursor().unwrap().is_none());
        let mut cursor = Cursor::start();
        cursor.next_page = 2;
        cache.write_cursor(&cursor).unwrap();
        assert_eq!(cache.read_cursor().unwrap().unwrap().next_page, 2);
